...
import { Button, Input, Alert, AlertTitle, Badge } from '@uibrium/ui';
    ...
    <div className="space-y-12 pb-20">
      <header className="space-y-4">
        <div className="flex items-center gap-2">
          <Badge variant="subtle" color="primary" className="rounded-full">Version 2.0</Badge>
        ...
        <p className="text-xl text-muted-foreground max-w-2xl leading-relaxed">
          UiBrium is a production-grade UI component library for building high-quality, 
          accessible web applications with React and Tailwind CSS.
        </p>
      </header>
        ...
        <Steps>
          <Step>
            ...
            <p className="text-muted-foreground mb-4">
              UiBrium requires <code className="font-mono text-primary bg-primary/5 px-1.5 py-0.5 rounded">framer-motion</code> for animations 
              and <code className="font-mono text-primary bg-primary/5 px-1.5 py-0.5 rounded">lucide-react</code> for icons.
            </p>
          ...
          <Step>
            ...
            <p className="text-muted-foreground mb-4">
              To ensure the component styles are correctly purged and loaded, add the UiBrium package 
...
export default {
  ...
  plugins: [],
          ...
          <Step>
            ...
            <p className="text-muted-foreground mb-4">
              Import the core UiBrium stylesheet in your main entry file (e.g., <code className="px-1.5 py-0.5 rounded bg-muted text-foreground">main.tsx</code> or <code className="px-1.5 py-0.5 rounded bg-muted text-foreground">_app.tsx</code>).
            </p>
            <div className="relative rounded-xl border bg-muted/50 p-6 font-mono text-sm group">
...
{`import '@uibrium/ui/styles.css';`}
          ...
          <Step>
            ...
            <p className="text-muted-foreground mb-4">
              Wrap your application with the <code className="px-1.5 py-0.5 rounded bg-muted text-foreground">ThemeProvider</code> to enable 
              automatic dark mode switching and system preference detection.
            </p>
            <div className="relative rounded-xl border bg-muted/50 p-6 font-mono text-sm group">
              <pre className="text-foreground">
  ...
  return (
    <ThemeProvider defaultMode="system">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <Card className="p-6 border-primary/10 bg-primary/5">
            ...
            <p className="text-muted-foreground text-sm">
              All interactive components strictly follow accessibility standards to ensure 
          ...
          <Card className="p-6 border-secondary/10 bg-secondary/5">
            ...
            <p className="text-muted-foreground text-sm">
              Powered by Tailwind CSS, resulting in lightning-fast initial page loads 
      ...
      <Callout icon={<Lightbulb className="h-5 w-5 text-warning" />} title="Pro Tip" type="default">
        ...
        found on most components to seamlessly merge UiBrium components with <code className="px-1.5 py-0.5 rounded bg-muted text-foreground">next/link</code> or other routing libraries.
      </Callout>

      <div className="flex items-center justify-between border-t pt-12">
        <div className="space-y-1">
          <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Previous</p>
          <Link href="/" className="text-lg font-bold text-primary hover:underline">Home Page</Link>
        </div>
        <div className="text-right space-y-1">
          <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Next</p>
          <Link href="/docs/components" className="text-lg font-bold text-primary hover:underline">Component Library</Link>
        </div>
      </div>
    </div>
  );

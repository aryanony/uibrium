...
import { Badge, Button, Card, CardHeader, CardTitle, CardDescription, CardContent, Tabs, TabsList, TabsTrigger, TabsContent } from '@uibrium/ui';
import { Terminal, Code, Cpu, Zap, Info, Layers } from 'lucide-react';
  ...
  const hooks = [
    {
      id: "use-disclosure",
      name: "useDisclosure",
      description: "A logic primitive for managing open/closed states (modals, drawers, accordions) with ease.",
...
export function Example() {
  ...
  return (
    ...
    },
    {
      ...
      description: "Detects interactions outside a target element, perfect for dropdowns and popovers.",
  ...
  return (
    <div className="space-y-12 max-w-4xl">
      ...
      <div className="space-y-6">
        <Badge variant="outline" className="rounded-full px-4 py-1 uppercase tracking-[0.2em] text-[10px] font-black border-primary"> Logic Primitives </Badge>
        <h1 className="text-4xl lg:text-5xl font-bold tracking-tighter leading-tight">Functional <span className="text-primary italic">Hooks</span>.</h1>
      ...
      <div className="grid gap-16">
        {hooks.map((hook) => (
          <section key={hook.id} id={hook.id} className="scroll-mt-24 space-y-8 border-t border-border pt-16 first:border-0 first:pt-0">
             ...
             <div className="space-y-4">
                <Badge variant="subtle" className="text-xs font-mono py-1 rounded-md bg-muted text-foreground">
                   {hook.code}
                </Badge>
                
                <Tabs defaultValue="usage" className="w-full">
                   <TabsList className="bg-muted/50 p-1 rounded-xl">
                      <TabsTrigger value="usage" className="rounded-lg px-6">Usage Blueprint</TabsTrigger>
                      <TabsTrigger value="anatomy" className="rounded-lg px-6">Technical Anatomy</TabsTrigger>
                   </TabsList>
                         ...
                         <CardContent className="p-8 space-y-4">
                            <div className="flex items-start gap-4 text-muted-foreground">
                               ...
                               <p className="text-sm italic leading-relaxed">
                                  This hook utilizes optimized event listeners and requestAnimationFrame for maximum performance. It is fully compatible with React 18 Concurrent Mode and Server Components.
                               </p>
                            </div>
                         </CardContent>
                      </Card>
      ...
      <div className="bg-primary/5 border border-primary/20 rounded-[2.5rem] p-12 lg:p-16 flex flex-col items-center text-center space-y-8">
         ...
         <Button asChild size="lg" className="rounded-full px-10 h-14 bg-primary hover:bg-primary/90 text-primary-foreground shadow-xl shadow-primary/20 transition-all duration-300">
